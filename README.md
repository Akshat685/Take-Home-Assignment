...
If you're doing a screen-share, here's a flow that covers everything in roughly 35–40 minutes:

1. **Demo the product** — enter a URL, crawl it, ask a few questions, point out the source links under each answer
2. **Crawler** — walk through `lib/crawler.ts`: URL normalization, BFS queue, hostname check, robots.txt, rate limiting, page limits
3. **Extraction** — `lib/extract.ts`: what gets removed, how the main content area is selected
4. **Chunking** — `lib/chunk.ts`: why word-based, the overlap strategy, what gets stored per chunk
5. **Indexing** — `app/api/crawl/route.ts` + `lib/embeddings.ts` + `lib/vector-store.ts`: how the crawl result becomes a searchable index
6. **RAG** — `lib/rag.ts`: the retrieval flow, the system prompt, why temperature is 0, how sources get deduplicated, not-found suppression
7. **UI** — `app/page.tsx`: loading/error states, how source links render under each message
8. **Honest limitations** — JS-heavy sites, in-memory storage, no streaming, rate limits on the free tier
9. **What's next** — streaming, persistent vector DB, Playwright fallback, reranking
...
app/
  api/
    chat/route.ts       ← handles questions, returns grounded answers
    crawl/route.ts      ← crawl + embed + index
  globals.css
  layout.tsx
  page.tsx              ← main UI with crawl form and chat
lib/
  chunk.ts              ← word-based chunking with overlap
  crawler.ts            ← BFS crawler with robots.txt + rate limiting
  embeddings.ts         ← Gemini embedding calls
  extract.ts            ← Cheerio-based boilerplate removal
  openai.ts             ← OpenAI client pointed at Gemini
  rag.ts                ← retrieval + answer generation
  robots.ts             ← robots.txt fetching and parsing
  types.ts              ← shared TypeScript types
  vector-store.ts       ← in-memory cosine similarity index
eval/
  examples.json         ← test sites and questions
  run.ts                ← eval runner
types/
  robots-parser.d.ts
.env.local.example
